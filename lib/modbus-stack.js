...
ModbusResponseStack.prototype._onData = function(chunk) {
  ...
  if (!this.requestHeader && this.bufferlist.length >= MBAP_LENGTH) {
    ...
    this.request = server.REQUESTS[this.functionCode](this.bufferlist);
    ...
    this.stream.removeListener('data', this._bindedOnData);
...
ModbusResponseStack.prototype.writeResponse = function() {
  if (!this._gotRequest) throw new Error('Can\'t call "writeResponse" until after the "request" event');
